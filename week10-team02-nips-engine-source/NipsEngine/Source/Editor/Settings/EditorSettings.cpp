...
namespace EditorKey
{
	...
	constexpr const char* CameraWASDAlwaysMove = "CameraWASDAlwaysMove";
...
void FEditorSettings::SaveToFile(const FString& Path) const
{
	...
	Viewport[EditorKey::CameraWASDAlwaysMove] = bCameraWASDAlwaysMove;
...
void FEditorSettings::LoadFromFile(const FString& Path)
{
	...
	if (Root.hasKey(EditorKey::Viewport))
	{
		...
		if (Viewport.hasKey(EditorKey::CameraWASDAlwaysMove))
			bCameraWASDAlwaysMove = Viewport[EditorKey::CameraWASDAlwaysMove].ToBool();
